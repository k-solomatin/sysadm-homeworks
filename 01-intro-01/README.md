...
### Домашнее Задание.
1. Скрины
- Terraform: ![Терраформ](dz/TF.png)
- Bash: ![bahs](dz/bash.png)
- Markdown: ![markdown](dz/md.png)
- Yaml: ![Yaml](dz/yaml.png)
- Jsonnet: ![Jsonnet](dz/json.png)
---
2. Описание решения задачи.
Менеджер по продукту ставит задачу.
Задача появляется на доске канбан у одного из разработчиков.
Devops готовит тестовую среду.
Разработчик создает ветку пишет код. Пушит на тестовую среду.
Devops подготавливает среду для автоматических тестов.
Тестировщик получает задачу - тестирует.
Менеджер получает решенную задячу - тестирует.
Devops подготавливает среду для деплоя в прод и создает "ручку" для быстрого отката.
Разрабочик катит в прод.
